...
test("ensureDaemon: remote destinations download the matching release themselves", async () => {
  const { host, execs, puts } = fakeHost({ health: "" });
  const events: EnsureEvent[] = [];
  await ensureDaemon(host, cfg({ target: "ssh", onLog: (e) => events.push(e) }));

  expect(puts.length).toBe(0); // no SDK-host download/upload when the destination can run curl
  expect(events.map((e) => e.phase)).toEqual(["connect", "probe", "download", "launch", "ready"]);
  const launch = execs.find((s) => s.includes("MINDWIRE_READY")) ?? "";
