...
async function resolveBinary(explicit?: string): Promise<ResolvedBinary> {
  ...
  // Prebuilt binary shipped as a per-platform optional dependency
  // (mindwire-daemon-<platform>-<arch>), installed only on the matching host.
  try {
    ...
    // package not installed (dev, or unsupported platform) — fall through.
  }
