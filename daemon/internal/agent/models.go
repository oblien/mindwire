...
// ConfiguredModelCatalogModule refines ModelCatalogModule for an adapter whose
// runnable catalog providers depend on credentials the user has connected. It
// lets the API expose that exact set to the client without putting catalog data
// or provider-specific credential logic in the daemon core.
//
// An adapter implementing this interface is authoritative even when it returns
// an empty list: an empty result means it has no catalog-backed providers
// connected, not that the client should fall back to its static broad scope.
type ConfiguredModelCatalogModule interface {
	ConfiguredModelCatalogProviders(store CredStore) []string
}
