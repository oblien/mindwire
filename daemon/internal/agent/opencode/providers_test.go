...
func TestEnvOnlyProviderConnectExtendsOpenCodeAllowlist(t *testing.T) {
	cfg := t.TempDir()
	t.Setenv("XDG_CONFIG_HOME", cfg)
	path := providerConfigPath()
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
...
`
	...
	store := mapStore{}
	a := adapter{}
	p := agent.CustomProvider{EnvVar: "GOOGLE_GENERATIVE_AI_API_KEY"}
	if err := a.SetProvider(store, agent.MemoryUser, "", "google", p, "AIza-secret", nil); err != nil {
		t.Fatalf("SetProvider: %v", err)
	}
	// Reconnecting must not duplicate an allowlist entry.
	if err := a.SetProvider(store, agent.MemoryUser, "", "google", p, "", nil); err != nil {
		t.Fatalf("SetProvider (idempotent): %v", err)
	}
	...
	var enabled []string
	if err := json.Unmarshal(top["enabled_providers"], &enabled); err != nil {
		t.Fatalf("parse enabled_providers: %v", err)
	}
	if strings.Join(enabled, ",") != "ohh,google" {
		t.Fatalf("enabled_providers = %v, want [ohh google]", enabled)
	}
	if _, ok := top["model"]; !ok {
		t.Fatalf("allowlist update dropped unrelated model setting: %s", raw)
	}
}
