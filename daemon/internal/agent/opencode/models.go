...
var (
	...
	_ agent.ConfiguredModelCatalogModule = adapter{}
)
...
// ModelCatalogProviders is the static fallback for callers that do not have an
// adapter credential store. The daemon API uses ConfiguredModelCatalogProviders
// instead, so its picker is limited to providers the user actually connected.
func (adapter) ModelCatalogProviders() []string { return []string{"*"} }

// ConfiguredModelCatalogProviders returns the catalog-backed providers whose
// credentials Mindwire will pass to OpenCode. OpenCode itself still owns
// authentication and execution: this only tells the client which models.dev
// provider rows are valid choices when the CLI cannot enumerate them.
func (adapter) ConfiguredModelCatalogProviders(store agent.CredStore) []string {
	set := map[string]bool{}
	for id := range envOnlyConnections(store) {
		set[id] = true
	}
	// The legacy Auth surface is also an env-only OpenCode provider connection.
	if store != nil && strings.TrimSpace(store.Get(ckAPIKey)) != "" {
		if id := strings.TrimSpace(store.Get(ckProvider)); id != "" {
			set[id] = true
		}
	}
