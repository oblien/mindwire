...
func (adapter) SetProvider(store agent.CredStore, scope agent.MemoryScope, _ string, id string, p agent.CustomProvider, apiKey string, secrets map[string]string) error {
	...
	// Env-only connect: persist the credential(s) so ProviderEnvForRun re-exports them at run time. When
	// the user's OpenCode config has enabled_providers, it is a native allowlist: add this just-connected
	// provider to it or OpenCode will correctly hide/reject the provider despite receiving its key.
	if strings.TrimSpace(p.BaseURL) == "" {
		if store == nil {
			return fmt.Errorf("cannot store credentials for provider %q", id)
		}
		if err := ensureProviderEnabled(providerConfigPath(), id); err != nil {
			return err
		}
...
// ensureProviderEnabled adds id to OpenCode's existing enabled_providers allowlist. It deliberately does
// nothing when the user has no allowlist: OpenCode's default discovery remains untouched. Disconnecting
// also leaves the entry behind — it may have predated Mindwire or be intentional configuration, and a
// provider without credentials remains unusable anyway.
func ensureProviderEnabled(path, id string) error {
	if path == "" {
		return fmt.Errorf("cannot resolve opencode config home")
	}
	top, err := readTopLevel(path)
	...
	raw, restricted := top["enabled_providers"]
	if !restricted {
		return nil
	}
	var enabled []string
	if err := json.Unmarshal(raw, &enabled); err != nil {
		return fmt.Errorf("parse enabled_providers: %w", err)
	}
	...
	updated, err := json.Marshal(enabled)
	if err != nil {
		return err
	}
	top["enabled_providers"] = updated
	out, err := json.MarshalIndent(top, "", "  ")
	if err != nil {
		return err
	}
