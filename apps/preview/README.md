...
A session-based product preview for MindWire — what a SaaS built on the MindWire SDK feels like. A
...
```bash
bun dev            # daemon (:8790) + preview (Vite :5174 → Hono :8787); open http://127.0.0.1:5174
```

Or run just this app (expects a daemon reachable at `DAEMON_URL`):

```bash
bun --filter='@mindwire/preview' run dev
```
...
```bash
bun --filter='@mindwire/preview' run build   # vite build → dist/  +  tsup → dist-server/
bun --filter='@mindwire/preview' run start   # node dist-server/index.js  (serves the SPA + API on one port)
```
