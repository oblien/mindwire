#!/usr/bin/env node
// One-command dev for the preview app: brings up the Hono API/SSE server (tsx watch, :8787) AND the
// Vite dev server (:5174, which proxies /api + /events to Hono). Open http://127.0.0.1:5174.
//
// The daemon the preview drives is NOT started here — point at one with DAEMON_URL (default
...
function run(name, cmd, args, env) {
  ...
  p.on("error", (e) => {
    console.error(`[preview] failed to start ${name}: ${e.code === "ENOENT" ? `'${cmd}' not found on PATH` : e.message}`);
    stopAll(1);
  });
  p.on("exit", (code, signal) => {
    if (shuttingDown) return;
    console.log(`\n[preview] ${name} exited (${signal || code}); stopping the rest…`);
    stopAll(code ?? 0);
  });
  procs.push(p);
}
...
if (isCloud) {
  console.log(`[preview] mode   → cloud / SaaS   (empty fleet on sign-in; wire a runtime from the Console — run the daemon with \`bun dev:demon\`)`);
}
console.log(`[preview] server → http://127.0.0.1:${serverPort}   (drives daemon at ${daemonUrl})`);
run("server", "bunx", ["tsx", "watch", "server/index.ts"], { PORT: serverPort, DAEMON_URL: daemonUrl });

console.log(`[preview] app    → http://127.0.0.1:5174   ← open this`);
