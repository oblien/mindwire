...
app.get("/api/ping", (c) => c.json({ ok: true, service: "preview" }));
...
initAuth()
  .then(() => {
    serve({ fetch: app.fetch, port: env.port }, (info) => {
      console.log(`[preview] listening on http://127.0.0.1:${info.port} (daemon: ${env.daemonUrl})`);
    });
  })
  .catch((err: unknown) => {
    console.error("[preview] auth init failed:", err instanceof Error ? err.message : err);
    process.exit(1);
  });
