 ...
 *
 * When the agent's native list is EMPTY (it can't scriptably enumerate its models, e.g. Codex), the list
 * is SOURCED from the catalog by the providers the agent declares it targets (`AgentInfo.modelProviders`):
...
export async function modelsForAgent(mw: Mindwire): Promise<ModelInfo[]> {
  ...
  // Empty native list → source the picker from the catalog by the agent's declared provider scope.
  const scope = (await mw.agent()).modelProviders ?? [];
  if (scope.includes("*")) {
    return catalog.map((m) => (m.provider ? { ...m, id: `${m.provider}/${m.id}` } : m));
  }
  if (scope.length) {
    const wanted = new Set(scope);
    return catalog.filter((m) => m.provider && wanted.has(m.provider));
  }
  return native;
}
