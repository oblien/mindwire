...
// Skips gracefully if the catalog hasn't been built.
import fs from "node:fs";
import path from "node:path";

// Prefer the tracked, committed catalog (published by daemon/build.sh) so the web build needs no Go
// toolchain; fall back to the local dist channel for dev.
const CANDIDATES = [
  path.resolve("../../daemon/catalog.json"),
  path.resolve("../../daemon/dist/latest/catalog.json"),
];
const CATALOG = CANDIDATES.find((p) => fs.existsSync(p));
const OUT = path.resolve("content/docs/reference/agents");

if (!CATALOG) {
  console.log("gen-catalog-docs: no daemon/catalog.json found — skipping (run daemon/build.sh)");
  process.exit(0);
}
