 ...
 *   5. Write it into packages/sdk/package.json
 ...
 * + GitHub release, and publishes the `mindwire-daemon-*` packages then `mindwire` to npm. Tags
 * containing `-` (rc.N, beta.N) become prereleases automatically (published to the npm `next` tag).
 *
 * Version model: the SDK's package.json version is the source of truth for npm and the tag. The Go
 * daemon binary carries its OWN, decoupled version (`agent.Version` in daemon/internal/agent/agent.go);
 * this script never touches it — it only surfaces it in the summary so you remember to bump it (edit the
 * const, then `daemon/build.sh`) when the daemon itself changed.
 */
...
// The daemon binary's own, decoupled version (a Go const). Read for the summary; never written here.
...
log(`Daemon binary version:  ${daemonVersion}  ${dim("(independent; bump in daemon/internal/agent/agent.go + ./build.sh if the daemon changed)")}`);
log(``);
...
/** The daemon binary's own version (Go const `agent.Version`), for the summary. Non-fatal: if the file
 *  or const can't be read, report it as unknown rather than aborting a release that doesn't touch it. */
...
async function runWizard(): Promise<string[] | null> {
  ...
  try {
    ...
    console.log(`  daemon binary    ${readDaemonVersion()}${dim("  (decoupled)")}`);
